...
#region Field Members
        private Lazy<Dictionary<string, ISetter<T>>> _setters = new Lazy<Dictionary<string, ISetter<T>>>(GetSetDelegates, LazyThreadSafetyMode.ExecutionAndPublication);
        private Lazy<Dictionary<string, IGetter<T>>> _getters = new Lazy<Dictionary<string, IGetter<T>>>(GetGetDelegates, LazyThreadSafetyMode.ExecutionAndPublication);
#endregion
...
#region Private Members
        ...
        private static ISetter<T> CreateSetterDelegateHelper<P>(string memberName)
        {
            ...
            return new Setter{Name = memberName, Set = Expression.Lambda<Action<T, object>>(safeAssign, instance, input).Compile()};
        }
        ...
        private static IGetter<T> CreateGetterDelegateHelper<P>(string memberName)
        {
            ...
            return new Getter{Name = memberName, Get = Expression.Lambda<Func<T, object>>(convert, instance).Compile()};
        }
